...
for reservoir_num = 1:Reservoir_num
    ...
    if reservoir_num > 1
        %%%%% tmp start %%%%%
        % display("b4 size(Reservoir) = " + size(Reservoir)); % check
        % display("size(RCtR) = " + size(RCtR)); % check
        % display("size(RCtR') = " + size(RCtR')); % check
        % display("size(RtR) = " + size(RtR)); % check
        %%%%% tmp end %%%%%
    ...
    %%%%% tmp start %%%%%
    % display("aft size(Reservoir) = " + size(Reservoir)); % check
    % display("rank(Reservoir) = "+ rank(Reservoir))
    %%%%% tmp end %%%%%
    ...
    %%%%% tmp start %%%%%
    % display("size(Wout1) = " + size(Wout1)); % check
    % display("size(Wout2) = " + size(Wout2)); % check
    % display("size(Wout3) = " + size(Wout3)); % check
    %%%%% tmp end %%%%%
    
    Wout = [Wout1,Wout2,Wout3];  %40*3
    %%%%% tmp start %%%%%
    % display("size(Wout) = " + size(Wout)); % check
    %%%%% tmp end %%%%%
    ...
    %%%%%%%%%%
    % [tmp_a1, tmp_b1] = size(Reservoir); % check 
    % display("size(Reservoir) = " + tmp_a1 + ", " + tmp_b1); % check
    % display("size(Target_R) = " + size(Target_R)); % check
    % display("size(Target_G) = " + size(Target_G)); % check
    % display("size(Target_B) = " + size(Target_B)); % check
    %%%%%%%%%%
